...
public class DepthFirstOrder<V> {
    ...
    private final List<V> visited = new LinkedList<>();
    ...
    public DepthFirstOrder(DirectedGraph<V> g) {
        ...
        for(V vertex : myGraph.getVertexSet()){
            if(!visited.contains(vertex)){
                depthFirstOrderR(vertex);
                numberOfDFTrees++;
            }
        }
    }

    private void depthFirstOrderR(V vertex){
        visited.add(vertex);
        preOrder.add(vertex);
        for(V neighbour : myGraph.getSuccessorVertexSet(vertex)){
            if(!visited.contains(neighbour)){
                depthFirstOrderR(neighbour);
            }
        }
